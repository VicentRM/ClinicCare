<template>
  <div>
    ...
    <div class="alert alert-danger" role="alert" v-show="showAlert">{{msgError}}</div>
    ...
    <table class="table table-sm table-striped table-bordered">
      ...
      <tbody>
        <tr v-for="tipo in arrayTipoPruebas" :key="tipo.id">
          <td>{{ tipo.id }}</td>
          <td>{{ tipo.descripcion }}</td>
          <td style="width: 20%;">
            <button type="button" class="btn btn-warning btn-sm">
              <i class="fa fa-pencil" aria-hidden="true" @click="cargarTipo(tipo)"></i>
            </button>
...
export default {
  ...
  methods: {
    ...
    obtenerTipos() {
      ...
      promise
        .then((response) => {
          ...
          this.arrayTipoPruebas = response.data;
        })
        .catch((error) => {
    ...
    guardarTipo() {
      ...
      const promise = axios.post("/tipopruebas", this.tipoEdit);
      promise
        .then((response) => {
    ...
    cargarTipo(data) {
      ...
      const promise = axios.get("/tipopruebas/buscar/" + this.update);
      promise
        .then((response) => {
          console.log(response.data);
    ...
    borrarTipo(data) {
      if (confirm("¿Seguro que deseas borrar este tipo de prueba?")) {
        const promise = axios.delete("/tipopruebas/" + data.id);
        promise
          .then((response) => {
            this.obtenerTipos();
          })
